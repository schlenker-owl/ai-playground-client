...
export const api = {
  ...
  chat: (
    user: string,
    system?: string,
    opts?: { temperature?: number; max_new_tokens?: number },
  ) =>
...
export const cv = {
  ...
  inferImage: (data: FormData) =>
    fetch('/api/cv/infer/image', { method: 'POST', body: data }).then(
      async (r) => {
  ...
  startVideo: (data: FormData) =>
    fetch('/api/cv/jobs/infer_video', { method: 'POST', body: data }).then(
      async (r) => {
        if (!r.ok) throw new Error(await r.text());
        return r.json();
      },
  ...
  job: (id: string) =>
    jfetch<{ status: string; out_dir?: string; error?: string }>(
      `/cv/jobs/${id}`,
    ),
  ...
  artifact: (path: string) =>
    jfetch<{ kind: string; content: string }>(
      `/cv/artifact?path=${encodeURIComponent(path)}`,
    ),
};

// --- Diffusion API client ---
export const diffusion = {
  sampleMnist: (payload: { n?: number; nrow?: number; seed?: number }) =>
    jfetch<{ image: string; n: number; T: number; out_path?: string }>(
      '/diffusion/mnist/sample',
      {
        method: 'POST',
        body: JSON.stringify(payload),
      },
